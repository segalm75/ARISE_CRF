...
%
% CALLING SEQUENCE:
%   [c] = genModCloudProf(prof,dprof,allprof,daystr,doy,ctype,cReff);
%
% INPUT:
%  - prof is a struct containing vertical profile data (from either model/aircraft)
%  - dprof is profile number within each day
%  - allprof is consecutive profile number
%  - daystr is date
%  - ctype is cloud type 'wc'/'ic'
%  - cReff is Reff 10,15,20 for 'wc', 15,20,30 for 'ic'
% 
% 
% OUTPUT:
...
% ---------------------------------------------------------------------------
%% function routine
function [c] = genModCloudProf(prof,dprof,allprof,daystr,doy,ctype,cReff)
