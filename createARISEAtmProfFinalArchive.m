...
%  NAME:
% createARISEAtmProfFinalArchive
%----------------------------------
% PURPOSE:
...
%
% CALLING SEQUENCE:
%  out = createARISEAtmProfFinalArchive
%
% INPUT:
...
% 
% - arise-C130-Hskping_c130_20140830_RA_Preliminary.ict air data
% - arise-probes.csv files generated from .ict files
% - arise star.mat files
%
%
% EXAMPLE:
%  - out = createARISEAtmProfFinalArchive
...
% MS: 2015-12-22, modified reateARISEAtmProf_wMet to assimilate latest
%                 archive data
% MS: 2015-12-23, added IRsurfT into cloud saved fields
% -------------------------------------------------------------------------
%% function routine
function out = createARISEAtmProfFinalArchive
version_set('1.1');
startup_plotting;
...
% read all data
    for i=1:length(air_files)
       ...
       %dates(i) = dm{:,i}(2);
        ...
        air.(dates_str).poten_Temp    = tmp.Potential_Temp;%[C]
        air.(dates_str).DewPoint      = tmp.Dew_Point + 273;%[in C, converted to K]
...
%         end
        ...
        is_params = [5:39];%{'TWC_gm3','LWC1_gm3','LWC2_gm3','nCDP_cm3','vCDP_um3cm3','CDP03_dNdlogD','CDP08_dNdlogD','CDP15_dNdlogD','CDP20_dNdlogD','CDP25_dNdlogD','CDP30_dNdlogD'};
        is_labels = {'TWC_gm3','LWC1_gm3','LWC2_gm3','nCDP_cm3','vCDP_cm3','CDP03um','CDP04um','CDP05um','CDP06um','CDP07um','CDP08um','CDP09um','CDP10um',...
    ...
    for j=1:profnum
                ...
                air.(dates_str).(prof_str).WindD =...
                     air.(dates_str).WindD(air.(dates_str).UTChr<=ut2 &...
                                                                     air.(dates_str).UTChr>=ut1);
                air.(dates_str).(prof_str).IRsurfT =...
                     air.(dates_str).IRsurfT(air.(dates_str).UTChr<=ut2 &...
                                                                     air.(dates_str).UTChr>=ut1);                                                                                                                 
...
%% save movies
movie2avi(Tempmov, ['ARISEmeanTempprof_' datestr(now,'yyyy-mm-dd') '.avi'], 'compression','None', 'fps',1);
winopen(['ARISEmeanTempprof_' datestr(now,'yyyy-mm-dd') '.avi']);
movie2avi(RHmov,   ['ARISEmeanRHprof_' datestr(now,'yyyy-mm-dd') '.avi']  , 'compression','None', 'fps',1);
winopen( ['ARISEmeanRHprof_' datestr(now,'yyyy-mm-dd') '.avi']);

%% save processed air struct for all flights
disp(strcat('saving',airdir,'ARISEairprocessed_' ,datestr(now,'yyyy-mm-dd')));
%airdir='F:\ARISE\C-130data\Met\SeaIceProfiles\';
save([airdir,'ARISEairprocessed_' datestr(now,'yyyy-mm-dd')],'-struct','air');
return;
